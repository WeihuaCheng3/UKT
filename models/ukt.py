...
class UKT(nn.Module):
    def __init__(self, n_question, n_pid, 
            ...
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=False, use_CL = True,use_mean_cov_diff=False, cl_weight=0.02, use_uncertainty_aug=True, l2=1e-5, emb_type="qid", emb_path="", pretrain_dim=768):
        super().__init__()
        """
        Input:
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        if emb_type in ["qid", "qidaktrasch", "qid_scalar", "qid_norasch"]:
            ...
            # 再加一道 self.model
            mean_d_output,cov_d_output = self.model(q_mean_embed_data,q_cov_embed_data, qa_mean_embed_data,qa_cov_embed_data,train)

            if train and self.use_CL:
                mean_d2_output, cov_d2_output = self.model(mean_q_aug_embed_data,cov_q_aug_embed_data, mean_qa_aug_embed_data,cov_qa_aug_embed_data,train)
                mas = mask
            ...
            activation = nn.ELU()
            # print(torch.mean(torch.mean(activation(cov_d_output)+1,dim = -1),-1))
...
class Architecture(nn.Module):
    ...
    def forward(self, q_mean_embed_data,q_cov_embed_data, qa_mean_embed_data, qa_cov_embed_data, train):
    ...
    def forward(self, mask, query_mean, query_cov, key_mean, key_cov, values_mean, values_cov, apply_pos=True):
        ...
        if mask == 0:  # If 0, zero-padding is needed.
            ...
            query2_mean, query2_cov = self.masked_attn_head(
                query_mean, query_cov, key_mean, key_cov, values_mean, values_cov, mask=src_mask, zero_pad=True # 只能看到之前的信息，当前的信息也看不到，此时会把第一行score全置0，表示第一道题看不到历史的interaction信息，第一题attn之后，对应value全0
            )
